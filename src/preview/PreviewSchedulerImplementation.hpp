...
namespace PreviewSchedulerDetail {

inline PreviewResult unavailableResult(const QString& detail = {}) {
...
inline PreviewScheduler::PreviewScheduler(QString helperPath, QObject* parent)
    : QObject(parent)
...
inline PreviewScheduler::~PreviewScheduler() {
...
inline PreviewScheduler& PreviewScheduler::instance() {
    static PreviewScheduler scheduler;
    return scheduler;
}

inline QString PreviewScheduler::defaultHelperPath() {
    return QDir(QCoreApplication::applicationDirPath())
        .filePath(QStringLiteral("ryofiles-preview-helper"));
}

inline void PreviewScheduler::configureLane(LaneState& state, int capacity, int timeoutMs) {
...
inline PreviewScheduler::LaneState& PreviewScheduler::stateFor(Lane lane) {
    return lane == Lane::InteractivePreview ? m_interactive : m_longWork;
}

inline const PreviewScheduler::LaneState& PreviewScheduler::stateFor(Lane lane) const {
    return lane == Lane::InteractivePreview ? m_interactive : m_longWork;
}

inline bool PreviewScheduler::submit(
    Lane lane,
    QObject* owner,
...
inline void PreviewScheduler::cancelOwner(QObject* owner) {
    if (!owner)
        return;
...
inline int PreviewScheduler::pendingCount(Lane lane) const {
    const LaneState& state = stateFor(lane);
    return state.pending.size() + (state.active.has_value() ? 1 : 0);
}

inline bool PreviewScheduler::helperRunning(Lane lane) const {
    return stateFor(lane).process.state() != QProcess::NotRunning;
}

inline void PreviewScheduler::ensureProcess(LaneState& state) {
...
inline void PreviewScheduler::dispatchNext(LaneState& state) {
    ...
    if (line.size() > PreviewProtocol::kMaxRequestLineBytes) {
        completeActive(
            state,
            PreviewSchedulerDetail::unavailableResult(
                QStringLiteral("Preview request is too large")));
        return;
    }
    line.append('\n');
...
inline void PreviewScheduler::handleReadyRead(LaneState& state) {
...
inline void PreviewScheduler::handleProcessFinished(LaneState& state) {
    state.requestTimer.stop();
    state.readBuffer.clear();
    ...
    if (state.active) {
        ...
        if (!intentional && failed.owner && failed.retries < 1) {
            ...
            failed.callback(PreviewSchedulerDetail::unavailableResult());
        }
    }
...
inline void PreviewScheduler::handleFailedToStart(LaneState& state) {
    state.requestTimer.stop();
    state.readBuffer.clear();
    state.stopping = false;
    state.intentionalStop = false;
...
inline void PreviewScheduler::handleRequestTimeout(LaneState& state) {
    if (!state.active)
        return;
    failActiveAndRestart(state, QStringLiteral("Preview timed out"), false);
}

inline void PreviewScheduler::completeActive(LaneState& state, PreviewResult result) {
    state.requestTimer.stop();
    if (!state.active)
        return;
...
inline void PreviewScheduler::failActiveAndRestart(
    LaneState& state,
    const QString& error,
    bool allowRetry) {
    state.requestTimer.stop();
    if (state.active) {
        ...
        if (allowRetry && failed.owner && failed.retries < 1) {
            ...
            failed.callback(PreviewSchedulerDetail::unavailableResult(error));
        }
    }
    stopProcess(state, false);
}

inline void PreviewScheduler::stopProcess(LaneState& state, bool intentional) {
    state.idleTimer.stop();
    state.requestTimer.stop();
    state.readBuffer.clear();
...
inline void PreviewScheduler::scheduleIdleExit(LaneState& state) {
    if (state.process.state() == QProcess::Running && !state.active && state.pending.isEmpty())
        state.idleTimer.start(kIdleExitMs);
}

inline void PreviewScheduler::failAllPending(LaneState& state, const QString& error) {
    while (!state.pending.isEmpty()) {
        Request request = state.pending.dequeue();
        if (request.owner && request.callback)
            request.callback(PreviewSchedulerDetail::unavailableResult(error));
    }
}
